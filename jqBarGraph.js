 ...
 * @param showLabel: false // show label above each bar
 ...
 * @param legend // object for show legend
 *
 * @example $('#divForGraph').jqBarGraph({ 
 * data: [
 *      {
 *          value: 4,
 *          bottomColor: '#6393c1',
    ...
    $.fn.jqBarGraph = $.fn.jqbargraph = function(options){
        ...
        doGraph = function(el){
            ...
            for(var val in data){
                ...
                if (valueData instanceof Array)
                    ...
                    value = valueData;
                lbl = data[val].lbl;
                bottomColor = data[val].bottomColor;
                topColor = data[val].topColor;
                unique = val+el.id; //unique identifier



                if (typeof lbl == 'undefined') lbl = arr.lbl;

                out  = "<div class='graphField"+el.id+"' id='graphField"+unique+"' style='position: absolute'>";
                if(typeof arr.showLabel != 'undefined' && arr.showLabel)
                    out += "<div class='graphValue"+el.id+"' id='graphValue"+unique+"'>"+prefix+value+postfix+"</div>";

                out += "<div class='graphBar"+el.id+"' id='graphFieldBar"+unique+"' style='background:-webkit-gradient(linear, left top, left bottom, color-stop(0%,"+ topColor + "), color-stop(100%,"+ bottomColor+ "));position: relative; overflow: hidden;'></div>";
                ...
                out += "</div>";
                if($('#graphFieldBar'+unique).length == 0) // check for already exist element
                    $(el).append(out);
                else
                    $('.graphValue'+el.id).html(prefix+value+postfix)
                //size of bar
                ...
                if(!arr.legends)
                    leg.push([ bottomColor, lbl, el.id, unique ]);
            ...
            if(arr.legend){
                if($('#legendHolder'+unique).length == 0)
                    $(el).append("<div id='legendHolder"+unique+"'></div>");
                $('#legendHolder'+unique).css({ 'width': legendWidth, 'float': 'right', 'text-align' : 'left'});
                $('#legendHolder'+unique).html(legend);
                $('.legendBar'+el.id).css({ 'float':'left', 'margin': 3, 'height': 12, 'width': 20, 'font-size': 16});
            }
        ...
        createLegend = function(legendArr){
            ...
            for(var val in legendArr){
                legendData = data[val]['legend'];
                if(typeof legendData != 'undefined') {
                    text = (legendData['prefix']||'') + (legendData['text']||data[val]['value']||'') +(legendData['postfix']||'')
                    legend += "<div id='legend"+legendArr[val][3]+"' style='overflow: hidden; zoom: 1;'>";

                    legend += "<div class='legendBar"+legendArr[val][2]+"' id='legendColor"+legendArr[val][3]+"' style='background-color:"+ (legendData['bar']?(legendData.barBGColor || legendArr[val][0]):'none')+"'></div>";
                    legend += "<div class='legendLabel"+legendArr[val][2]+"' id='graphLabel"+unique+"' style='" + (legendData.style||'') + "'>"+text+"</div>";
                    legend += "</div>";
                }
            }
